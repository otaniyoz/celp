...
image both_images(image a, image b) {
  image both =
      make_image(a.c > b.c ? a.c : b.c, a.h > b.h ? a.h : b.h, a.w + b.w);
  int i, j, k;
  for (k = 0; k < a.c; ++k) {
    for (j = 0; j < a.h; ++j) {
      for (i = 0; i < a.w; ++i) {
...
image draw_matches(image a, image b, match *matches, int n, int inliers) {
  ...
  for (i = 0; i < n; ++i) {
    ...
    for (j = bx; j < ex + a.w; ++j) {
      ...
      set_pixel(both, 0, r, j, i < inliers ? 0.31 : 1);
      set_pixel(both, 1, r, j, i < inliers ? 0.98 : 0.33);
      set_pixel(both, 2, r, j, 0.33);
    }
  }
  return both;
}
...
match *match_descriptors(descriptor *a, int an, descriptor *b, int bn,
  ...
  *mn = an;
  ...
  for (j = 0; j < an; j++) {
    ...
    min = l1_distance(a[j].data, b[0].data, a[j].n);
    for (i = 0; i < bn; i++) {
  ...
  int *removed = calloc(an, sizeof(int));
  for (i = 0; i < an; i++) {
    if (!seen[m[i].bi]) {
      seen[m[i].bi] = 1;
      // m[count] = m[i];
...
int model_inliers(matrix H, match *m, int n, float thresh) {
  ...
  int count = 0;
  match inlier;
  // count number of matches that are inliers i.e. distance(H*p, q) < thresh
    ...
    for (j = topleft.y; j < botright.y; j++) {
      for (i = topleft.x; i < botright.x; i++) {
        ...
        if (p.x >= 0 && p.x < b.w && p.y >= 0 && p.y < b.h)
          set_pixel(c, k, j - dy, i - dx, bilinear_interpolate(b, k, p.y, p.x));
      }
    }
  }
  return c;
...
image panorama_image(image a, image b, float sigma, float thresh, int nms,
  ...
  if (0) {
    ...
    save_image(inlier_matches, "results/inliers");
  }
...
image cylindrical_project(image im, float f) {
  ...
  image c = make_image(im.c, im.h, im.w); // copy_image(im);
  for (i = 0; i < c.w; i++) {
    for (j = 0; j < c.h; j++) {
      for (k = 0; k < c.c; k++) {
        float xb = f * atan2f(i - xc, f) + xc;
        float yb = f * (j - yc) / sqrtf((i - xc) * (i - xc) + f * f) + yc;
        set_pixel(c, k, floor(yb + 0.5), floor(xb + 0.5),
                  get_pixel(im, k, j, i));
      }
    }
  }
...
// TODO: implement stereographic projection
